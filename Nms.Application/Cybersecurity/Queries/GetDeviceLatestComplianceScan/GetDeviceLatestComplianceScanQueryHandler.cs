...
public class GetDeviceLatestComplianceScanQueryHandler : IRequestHandler<GetDeviceLatestComplianceScanQuery, DeviceComplianceScanDto?>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetDeviceLatestComplianceScanQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<DeviceComplianceScanDto?> Handle(GetDeviceLatestComplianceScanQuery request, CancellationToken cancellationToken)
    {
        var scan = await _unitOfWork.DeviceComplianceScans.GetLatestScanForDeviceAsync(request.DeviceId, cancellationToken);
        if (scan == null)
            return null;
